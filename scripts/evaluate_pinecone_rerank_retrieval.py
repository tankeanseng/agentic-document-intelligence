...
from scripts.pinecone_rerank_retrieval import apply_rerank, build_rerank_documents
...
def evaluate_matches(matches, expected_terms, expected_sections):
...
def write_report(project_root: Path, run_id: str, report):
    out_path = (
        project_root
        / "artifacts"
        / "experiments"
        / run_id
        / "retrieval"
        / "pinecone_rerank_eval_report.json"
    )
...
def main():
    parser = argparse.ArgumentParser(description="Run a gold-query evaluation over Pinecone reranked retrieval.")
    parser.add_argument("--project-root", default=None)
    parser.add_argument(
        "--chunk-input",
        default="artifacts/experiments/component2_chunk_generation/chunks/microsoft_fy2025_10k_summary_chunks.json",
    )
    parser.add_argument("--run-id", default="component2_pinecone_rerank_eval")
    ...
    for case in GOLD_QUERIES:
        ...
        evaluation = evaluate_matches(reranked, case["expected_terms"], case["expected_sections"])
        cases.append(
            {
                "query": case["query"],
                ...
                "top_sections": [match["metadata"].get("section_title", "") for match in reranked[:3]],
                "top_child_snippets": [match["child_text"][:220] for match in reranked[:3]],
