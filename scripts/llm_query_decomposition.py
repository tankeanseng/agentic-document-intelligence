...
- "Explain Microsoft's FY2025 revenue growth and identify who the CEO is."
  -> ["Explain Microsoft's FY2025 revenue growth?", "Who is the CEO of Microsoft?"]
- "Revenue, operating income, and cash flow for FY2025 please."
  -> ["What was Microsoft's FY2025 revenue?", "What was Microsoft's FY2025 operating income?", "What was Microsoft's FY2025 cash flow?"]
- "Could you tell me the CEO of Microsoft and summarize fiscal 2025 cybersecurity risks?"
  -> ["Who is the CEO of Microsoft?", "Summarize Microsoft's fiscal 2025 cybersecurity risks?"]
"""
...
def decompose_query_with_llm(query: str, model: str = MODEL_NAME, client: OpenAI | None = None) -> dict[str, Any]:
    load_project_env()
...
def write_report(project_root: Path, run_id: str, query: str, result: dict[str, Any], model: str) -> Path:
    ...
    report_path = output_dir / "llm_query_decomposition_report.json"
    report = {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run LLM-assisted query decomposition.")
    parser.add_argument(
        "--query",
        default="Explain Microsoft's FY2025 revenue growth and identify who the CEO is.",
    )
    parser.add_argument("--model", default=MODEL_NAME)
    parser.add_argument("--run-id", default="component3_llm_query_decomposition")
    args = parser.parse_args()

    result = decompose_query_with_llm(args.query, model=args.model)
