...
def build_single_subquery_bundle(query: str) -> dict[str, object]:
    return {
        "original_query": query,
        "policy": {"forced_single_subquery_eval": True},
...
def evaluate_parallel_safe_case(result: dict[str, object], case: dict[str, object]) -> dict[str, object]:
    base_evaluation = evaluate_case(result, case)
    sub_query_results = result.get("sub_query_results", [])
    first_sub_query = sub_query_results[0] if sub_query_results else {}
    execution_policy = first_sub_query.get("execution_policy", {})
    expected_parallel_safe = bool(case.get("expect_parallel_safe", True))
    actual_parallel_safe = bool(execution_policy.get("parallel_safe", False))
    active_sources = list(execution_policy.get("active_sources", []))
    missing_active_sources = [
        source for source in case.get("required_sources", []) if source not in active_sources
    ]
    parallel_safe_mismatch = expected_parallel_safe != actual_parallel_safe
    passed = base_evaluation["passed"] and not missing_active_sources and not parallel_safe_mismatch
    return {
        **base_evaluation,
        "expected_parallel_safe": expected_parallel_safe,
        "actual_parallel_safe": actual_parallel_safe,
        "missing_active_sources": missing_active_sources,
        "parallel_safe_mismatch": parallel_safe_mismatch,
        "passed": passed,
    }
...
def write_report(project_root: Path, run_id: str, report: dict[str, object]) -> Path:
    output_path = (
        ...
        / run_id
        / "orchestration"
        / "parallel_safe_orchestration_eval_report.json"
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate parallel-safe SQL+graph orchestration on atomic mixed-source subqueries.")
    parser.add_argument(
        "--cases-path",
        default="evals/parallel_safe_orchestration_cases.json",
    )
    ...
    parser.add_argument("--run-id", default="component7_parallel_safe_orchestration_eval")
    ...
    for case in cases:
        ...
        transformed_bundle = build_single_subquery_bundle(case["query"])
        ...
        policy = result["sub_query_results"][0]["execution_policy"]
        case_results.append(
            {
                ...
                "active_sources": policy["active_sources"],
                "evaluation": evaluation,
            }
        )

    passed_count = sum(1 for item in case_results if item["evaluation"]["passed"])
    parallel_safe_count = sum(1 for item in case_results if item["parallel_safe"])
    report = {
